// This file has been edited to remove excel registrations.
...
// "" means it will be registered to appear in Excel with the same name as here in the code.
// "." means it will be registered to appear in Excel with the same name as here in the code, prefixed with "<this_dll_name>.".
// The below syntax requires C++ 11. It automatically creates a std::vector<const char*>() from the list of synonyms on the fly in the call to the overloaded function XLFunction::Declare().
